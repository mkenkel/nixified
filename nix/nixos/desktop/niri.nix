  ...
  programs.niri = {
    ...
    settings = {
      ...
      binds = {
        ...
        "Alt+Shift+H".action.spawn-sh =
          "pkill fuzzel || cliphist list | fuzzel --no-fuzzy --dmenu | cliphist decode | wl-copy"; # Not working yet
        "Alt+C".action.spawn-sh =
          "cat ~/.config/river/init | grep -i map | sed -e 's/riverctl//g' -e 's/map//g' -e 's/normal//g' | fuzzel --cache \"($XDG_CACHE_HOME/unicode)\" -w 100 --horizontal-pad 20 -di) || exit 0"; # Not working yet
